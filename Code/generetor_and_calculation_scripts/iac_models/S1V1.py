...
'''
elkDockerContainer = CClass(execution_environment, 'Elk Docker Container', stereotype_instances=[container_env_type])
...
add_links({elasticsearch: elkDockerContainer}, role_name="deployment_nodes", stereotype_instances=[deployed_in_container_type])
add_links({logstash: elkDockerContainer}, role_name="deployment_nodes", stereotype_instances=[deployed_in_container_type])
add_links({kibana: elkDockerContainer}, role_name="deployment_nodes", stereotype_instances=[deployed_in_container_type])
add_links({elkDockerContainer: elkServer}, role_name="to", stereotype_instances=[runs_on_deployment_node_relation_type])
'''
...
add_links({stack_web_app_mvc : dockerContainer_basket}, role_name="to", stereotype_instances=[defines_deployment_of_deployment_node_relation_type])
#add_links({stack_basket_api : [dockerContainer_payment]}, role_name="to", stereotype_instances=[defines_deployment_of_deployment_node_relation_type])
...
add_links({marketing_service: dockerContainer_catalog}, role_name="deployment_nodes", stereotype_instances=[deployed_on_type])
...
add_links({order_service: dockerContainer_basket}, role_name="deployment_nodes", stereotype_instances=[deployed_on_type])
...
add_links({payment_service: dockerContainer_basket}, role_name="deployment_nodes", stereotype_instances=[deployed_on_type])
...
#Views
'''
_all = CBundle("all", elements=[marketing_service, basket_service, order_service, locations_service, catalog_service, payment_service, payment_service, identity_service, web_client_gateway, mobile_gateway, event_bus, logstash, elkServer, elkDockerContainer, webserver0_0_0_0, kibana, elasticsearch ])
elk = CBundle("IaC Deployment", elements=[logstash, elkServer, elkDockerContainer, webserver0_0_0_0, kibana, elasticsearch])
eShopOnContainersMicroservice = CBundle("ES1", elements = [marketing_service, basket_service, order_service, locations_service, catalog_service, payment_service, payment_service, identity_service, web_client_gateway, mobile_gateway, event_bus])
#server1 = CBundle("Web Server", elements= [webserver0_0_0_0, awsServer, accountService, catalog, order, cart, payment, shippingService, login, accountDB, cartDB, orderDB, apiGateway, broker, qmaster, zipkin])
'''
_all = CBundle("S1V1", elements = elkServer.class_object.get_connected_elements())
S1V1Views = [_all, {},]
